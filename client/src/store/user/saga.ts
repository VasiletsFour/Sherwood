...
import {UserAdminQuery, UserAdminUpdate} from "../../request/UserApi";
...
export function* UserSaga() {
    while (true) {
        ...
        if (getAdminUserAction.trigger.is(action)) {
            yield call(getAdminUserWorker, action.query);
        }
...
function* getAdminUserWorker(query?:UserAdminQuery) {
    try {
        yield put(getAdminUserAction.running());
        const response = yield call(getAdminUserApi, query);
